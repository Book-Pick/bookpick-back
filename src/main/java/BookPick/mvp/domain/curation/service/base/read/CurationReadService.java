...
public class CurationReadService {
    ...
    // -- 큐레이션 단건 조회 --
    @Transactional
    public CurationGetRes findCuration(
            Long curationId, CustomUserDetails user, HttpServletRequest req, boolean isEdit) {
        ...
        Curation curation =
                curationRepository
                        .findByIdWithUserAndLock(curationId)
        ...
        if (user != null) {
            // 1. 좋아요 정보 찾기
            Optional<CurationLike> curationLike =
                    curationLikeRepository.findByUserIdAndCurationId(user.getId(), curationId);
            ...
            // 2. 큐레이터 구독 여부 조회
            isSubscribedCurator =
                    curationSubscribeService.isSubscribeCurator(
                            user.getId(), curation.getUser().getId());

            // 3. 큐레이션 작성자면 책 정보 넣어서 큐레이션 반환
            if (isEdit) {
                if (curation.getUser().getId().equals(user.getId())) {
                    return CurationGetRes.fromOwnerView(
                            curation, isSubscribedCurator, isLikedCuration);
                } else {
