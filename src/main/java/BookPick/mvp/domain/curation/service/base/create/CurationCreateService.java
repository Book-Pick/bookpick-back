...
public class CurationCreateService {
    ...
    public CurationCreateResult saveCuration(Long userId, CurationReq req) {
        ...
        // 발행
        if (!req.isDrafted()) {
            return CurationCreateResult.from(
                    publishNewCuration(userId, req), SuccessCode.CURATION_PUBLISH_SUCCESS);
        } else {
            return CurationCreateResult.from(
                    draftNewCuration(userId, req), SuccessCode.CURATION_DRAFT_SUCCESS);
        }
    }
