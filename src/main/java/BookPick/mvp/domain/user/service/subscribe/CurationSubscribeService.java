...
public class CurationSubscribeService {
    ...
    public CuratorSubscribeRes subscribe(Long userId, CuratorSubscribeReq req) {
        ...
        // 3. 구독할 유저  얻기
        User curator =
                userRepository.findById(req.curatorId()).orElseThrow(CuratorNotFoundException::new);

        // 4. 해당 유저가 구독했는지 체크
        // 3.1 유저의 큐레이션 구독 정보 가져오기
        Optional<CuratorSubscribe> opt =
                curationSubscribeRepository.findByUserIdAndCuratorId(userId, req.curatorId());

        // 3.2 구독 정보가 없으면 생성
        if (opt.isEmpty()) {
            CuratorSubscribe curationSubscribe =
    ...
    public boolean isSubscribeCurator(Long userId, Long CuratorId) {
        Optional<CuratorSubscribe> subInfo =
                curationSubscribeRepository.findByUserIdAndCuratorId(userId, CuratorId);
    ...
    public SubscribedCuratorPageRes getSubscribedCurators(Long userId, int page, int size) {
        ...
        Page<CuratorSubscribe> subscribesPage =
                curationSubscribeRepository.findByUserIdOrderByIdDesc(userId, pageable);

        List<SubscribedCuratorRes> content =
                subscribesPage.getContent().stream()
