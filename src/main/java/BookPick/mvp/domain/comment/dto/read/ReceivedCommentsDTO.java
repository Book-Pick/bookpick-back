...
public record ReceivedCommentsDTO(List<CommentDetailRes> comments) {
    public static ReceivedCommentsDTO from(List<Comment> comments) {
        List<CommentDetailRes> commentDetailResList =
                comments.stream().map(CommentDetailRes::of).collect(Collectors.toList());
