...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query(
            "SELECT c FROM Comment c LEFT JOIN FETCH c.parent LEFT JOIN FETCH c.user WHERE"
                    + " c.curation.id = :curationId AND c.deletedAt IS NULL")
    ...
    @Query(
            ...
            """)
    List<Comment> findLatestCommentsByUserId(@Param("userId") Long userId, Pageable pageable);
}
