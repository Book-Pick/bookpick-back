        ...
        try {
            ...
            if (jti != null && tokenBlacklistManager.isBlacklisted(jti)) {
                ...
                customAuthenticationEntryPoint.commence(
                        request, response, new AuthenticationException("Token logged out") {});
                return;
            }
            ...
            CustomUserDetails customUserDetails =
                    CustomUserDetails.fromJwt(userId, email, authorities);
            var auth =
                    new UsernamePasswordAuthenticationToken(
                            customUserDetails, null, customUserDetails.getAuthorities());
            auth.setDetails(
                    new WebAuthenticationDetailsSource()
                            .buildDetails(request)); // 추가 정보 저장 (어디서 접속했는지)
            SecurityContextHolder.getContext().setAuthentication(auth);

        } catch (JwtTokenExpiredException e) {
            request.setAttribute("exception", ErrorCode.TOKEN_EXPIRED);
            customAuthenticationEntryPoint.commence(
                    request, response, new AuthenticationException("Token expired") {});
            return;
        } catch (InvalidTokenTypeException e) {
            request.setAttribute("exception", ErrorCode.INVALID_TOKEN_TYPE);
            customAuthenticationEntryPoint.commence(
                    request, response, new AuthenticationException("Invalid token") {});
            return;
        }
        filterChain.doFilter(request, response);
    }
