...
class ReceivedCommentsServiceTest {
    ...
    @InjectMocks
    private ReceivedCommentsService receivedCommentsService;
    ...
    void receivedCommentsRead_success() {
        ...
        Comment comment1 = Comment.builder().id(1L).content("Comment 1").user(user).curation(curation).build();
        Comment comment2 = Comment.builder().id(2L).content("Comment 2").user(user).curation(curation).build();
        Comment comment3 = Comment.builder().id(3L).content("Comment 3").user(user).curation(curation).build();
        List<Comment> comments = List.of(comment1, comment2, comment3);

        when(commentRepository.findLatestCommentsByUserId(userId, PageRequest.of(0, 3))).thenReturn(comments);
    ...
    void receivedCommentsRead_noComments() {
        // given
        Long userId = 1L;
        when(commentRepository.findLatestCommentsByUserId(userId, PageRequest.of(0, 3))).thenReturn(Collections.emptyList());
