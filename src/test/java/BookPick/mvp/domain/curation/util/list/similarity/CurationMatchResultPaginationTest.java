...
class CurationMatchResultPaginationTest {
    ...
    void paginate_withCursor() {
        // given
        User dummyUser = User.builder().id(99L).build(); // Dummy user for CurationMatchResult builder
        List<CurationMatchResult> results = IntStream.range(0, 20)
                .mapToObj(i -> CurationMatchResult.builder()
        ...
        // when
        List<CurationMatchResult> paginated = CurationMatchResultPagination.paginate(results, cursor, pageable);
    ...
    void paginate_nullCursor() {
        ...
        // when
        List<CurationMatchResult> paginated = CurationMatchResultPagination.paginate(results, null, pageable);
    ...
    void paginate_cursorOutOfBounds() {
        // given
        User dummyUser = User.builder().id(99L).build(); // Dummy user for CurationMatchResult builder
        List<CurationMatchResult> results = IntStream.range(0, 10)
                .mapToObj(i -> CurationMatchResult.builder()
                        .curation(Curation.builder().id((long) i).build())
                        .user(dummyUser)
                        .totalMatchCount(i)
