...
@WebMvcTest(
        value = TokenRefreshController.class,
...
class TokenRefreshControllerTest {
    ...
    void refreshToken_success() throws Exception {
        ...
        // when & then
        mockMvc.perform(post("/api/v1/auth/token/refresh").with(user(userDetails)))
        ...
        verify(tokenRefreshService)
                .refreshTokens(any(CustomUserDetails.class), eq(oldRefreshToken));
        verify(refreshTokenCookieManager)
                .addRefreshTokenCookie(any(HttpServletResponse.class), eq("new_refresh_token"));
    }
    ...
    void refreshToken_fail_notAuthenticated() throws Exception {
        // when & then
        mockMvc.perform(post("/api/v1/auth/token/refresh")).andExpect(status().isUnauthorized());
    ...
    void refreshToken_newRefreshTokenCookieSet() throws Exception {
        ...
        // then
        verify(refreshTokenCookieManager)
                .addRefreshTokenCookie(any(HttpServletResponse.class), eq(newRefreshToken));
    }
    ...
    void refreshToken_serviceInvoked() throws Exception {
        ...
        // then
        verify(tokenRefreshService, times(1))
                .refreshTokens(any(CustomUserDetails.class), eq(refreshToken));
    }
}
